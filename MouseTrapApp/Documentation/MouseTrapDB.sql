...
BEGIN
    ...
    -- Fetch the Mapid from the provided Game ID
    SELECT Mapid INTO map_id
    FROM `Game`
    WHERE id = p_game_id;
    
    -- Fetch map dimensiosn from the Map Table using the Mapid
...
BEGIN
    ...
    SELECT Mapid INTO map_id FROM `Game` WHERE id = p_game_id;
    
    -- Select tile data for the map to populate the game board
